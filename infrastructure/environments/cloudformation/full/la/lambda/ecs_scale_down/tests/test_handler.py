...
from ecs_scale_down.handler import lambda_handler
import logging
...
class TestLambdaHandler(unittest.TestCase):
    ...
    def test_lambda_handler(self, mock_boto_client, mock_get_env):
        ...
        mock_ecs_client.update_service.assert_has_calls([
            call(cluster='my-cluster', service='dagit-service', desiredCount=0),
            call(cluster='my-cluster', service='daemon-service', desiredCount=0),
            call(cluster='my-cluster', service='code-server-service', desiredCount=0)
        ])
